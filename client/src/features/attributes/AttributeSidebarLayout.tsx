...
export const AttributeSidebarLayout: React.FC<AttributeSidebarLayoutProps> = ({
	...
	useEffect(() => {
		...
		if (query.length) {
			// chỉ search
			searchShopAttributes({ query });
		} else {
			// rỗng => lấy tất cả
			getShopAttributes();
		}
	}, [searchQuery, getShopAttributes, searchShopAttributes]);

	// ================= Sidebar items =================
	const attributes = shopAttributes?.attributes || [];
		...
		return (
			<div className="space-y-6">
				...
				<AttributeHeader
					...
					onUpdated={reloadShopAttributes}
					disableActions={valuePanelOpen}
				/>
				...
				<div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
					<div>
						...
						<p className="font-medium text-gray-800">
							{formatVNDate(selectedAttribute.createdAt)}
						</p>
					</div>
					<div>
						<p className="font-semibold flex items-center gap-1 text-indigo-600">
						...
						<p className="font-medium text-gray-800">
							{formatVNDate(selectedAttribute.updatedAt)}
						</p>
					</div>
					<div>
						<p className="font-semibold flex items-center gap-1 text-indigo-600">
						...
						<p
							className={`font-medium flex items-center gap-1 ${
								selectedAttribute.isActive ? "text-green-600" : "text-red-600"
							}`}>
					...
					<div>
						<p className="font-semibold flex items-center gap-1 text-indigo-600">
							<Globe className="w-4 h-4" /> Phạm vi {/* Hoặc dùng "Toàn cục" */}
						</p>
						<p className="font-medium text-gray-800">
							{selectedAttribute.isGlobal ? "Toàn cục" : "Cục bộ"}
						</p>
					</div>
				</div>
					...
					<div
						...
						{selectedAttribute.values.map((value) => (
							<CardAttributeValue
								...
								showStatus={editMode}
								onEdit={async () => await reloadShopAttributes()}
								onHide={async () => await reloadShopAttributes()}
								onDelete={async () => await reloadShopAttributes()}
							/>
						))}
					</div>
				) : (
