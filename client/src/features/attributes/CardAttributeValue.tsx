...
import {
	...
	Attribute,
} from "@shared/features/attribute";
...
export const CardAttributeValue: React.FC<CardAttributeValueProps> = ({
	...
	const handleSaveEdit = async () => {
		...
		const isImageRemoved = !previewUrl && value.image;
		...
		const res = await updateShopAttributeValue(value._id, payload);
		if (res.success) {
			setEditing(false);
	...
	const handleHide = () => {
		showConfirm({
			...
			onConfirm: async () => {
				const res = await toggleShopAttributeValue(value._id);
	...
	const handleDelete = () => {
		showConfirm({
			...
			onConfirm: async () => {
				const res = await deleteShopAttributeValue(value._id);
	...
	return (
		<div
			...
			} items-center bg-white border border-gray-200 rounded-xl shadow-md hover:shadow-xl transition-shadow duration-300 gap-2 w-full group`}>
			{/* Chỉ hiện khi không phải đang sửa, có cờ showStatus và đang bị ẩn (!isActive) */}
			{!editing && showStatus && !value.isActive && (
				<div className="absolute top-2 right-2 z-10">
					<span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide bg-red-50 text-red-600 border border-red-100 shadow-sm">
						Đang ẩn
					</span>
				</div>
			)}
			...
			{previewUrl || !mini ? (
				<div
					...
					} rounded-lg overflow-hidden border border-gray-200 flex items-center justify-center bg-gray-100 text-gray-400 cursor-pointer`}>
					{previewUrl ? (
						<img
							src={previewUrl}
							alt={editValue}
			...
			<div
				...
				{/* Nút action */}
				{!compact && !mini && showActions && value.shopId && (
