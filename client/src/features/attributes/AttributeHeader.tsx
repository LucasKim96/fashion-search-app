...
import { Input, GradientButton } from "@shared/core/components/ui";
...
export const AttributeHeader: React.FC<AttributeHeaderProps> = ({
	...
	const { updateShopAttributeLabel, deleteShopAttribute, hideShopAttribute } =
		useAttribute();
	...
	const saveLabel = async () => {
		...
		setLoadingEdit(true);
		try {
			await updateShopAttributeLabel(attribute._id, { label });
			setIsEditing(false);
	...
	const handleDelete = () => {
		showConfirm({
			message:
				"Bạn có chắc chắn muốn xóa thuộc tính và các giá trị thuộc tính này?",
			onConfirm: async () => {
				setLoadingDelete(true);
				try {
					await deleteShopAttribute(attribute._id);
					onUpdated?.();
				} finally {
	...
	const handleToggleStatus = () => {
		showConfirm({
			message: `Bạn có chắc chắn muốn ${
				attribute.isActive ? "ẩn" : "hiện"
			} thuộc tính và các giá trị thuộc tính này?`,
			onConfirm: async () => {
				setLoadingHide(true);
				try {
					await hideShopAttribute(attribute._id);
					onUpdated?.();
				} finally {
	...
	return (
		<div className="flex justify-between items-center">
			...
			{!disableActions && !attribute.isGlobal && (
