import os
os.environ['MPLBACKEND'] = 'Agg'  # phải đặt trước khi import matplotlib.pyplot
...
import matplotlib
import matplotlib.pyplot as plt  # bây giờ sẽ dùng backend 'Agg'
...
def make_balanced_weights_advanced(images, nclasses, mode="sqrt"):
    """
    Tạo trọng số cân bằng giữa các class, với 3 chế độ:
    - mode="original": công thức gốc (N / n_i)
    - mode="sqrt": làm dịu bằng căn bậc hai (sqrt(N / n_i))
    - mode="log": làm dịu bằng log (log1p(N / n_i))
    """
    ...
    N = float(sum(count))  # tổng số ảnh
    for i in range(nclasses):
        if count[i] == 0:
            weight_per_class[i] = 0
            continue
        ratio = N / float(count[i])
        if mode == "original":
            weight_per_class[i] = ratio
        elif mode == "sqrt":
            weight_per_class[i] = np.sqrt(ratio)
        elif mode == "log":
            weight_per_class[i] = np.log1p(ratio)
        else:
            raise ValueError("mode phải là 'original', 'sqrt' hoặc 'log'.")

    # --- Gán trọng số cho từng ảnh ---
