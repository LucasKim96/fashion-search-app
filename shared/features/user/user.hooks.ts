...
export const useUser = () => {
	...
	const updateBasicInfo = useCallback(
		async (id: string, payload: UpdateUserBasicInfoRequest) => {
			...
			if (res?.success) {
				await userByIdState.run(() => getUserByIdApi(id));
			}
	...
	const updateAvatar = useCallback(
		async (id: string, file: File) => {
			...
			const res = await avatarState.run(() => updateUserAvatarApi(id, file), {
				showToastOnSuccess: true,
			});

			// 2. Nếu upload thành công → reload user vừa sửa
			if (res?.success) await userByIdState.run(() => getUserByIdApi(id));
