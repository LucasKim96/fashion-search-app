...
import { USER_ENDPOINTS } from "@shared/core/constants/api.constants";
...
export const getAllUsersApi = async (): Promise<ApiResponse<UserInfo[]>> => {
	const res = await axiosInstance.get<ApiResponse<UserInfo[]>>(
...
export const searchUsersApi = async (
	...
	const res = await axiosInstance.get<ApiResponse<UserInfo[]>>(
		USER_ENDPOINTS.SEARCH(keyword)
	);
	return res.data;
};

/** Thống kê theo giới tính */
export const statsByGenderApi = async (): Promise<
	ApiResponse<UserStatsGender[]>
> => {
	const res = await axiosInstance.get<ApiResponse<UserStatsGender[]>>(
		USER_ENDPOINTS.STATS_GENDER
	);
	return res.data;
};

/** Thống kê theo độ tuổi */
export const statsByAgeRangeApi = async (): Promise<
	ApiResponse<UserStatsAgeRange>
> => {
	const res = await axiosInstance.get<ApiResponse<UserStatsAgeRange>>(
		USER_ENDPOINTS.STATS_AGE
	);
	return res.data;
};
