...
export const PublicProductCard: React.FC<PublicProductCardProps> = ({
	...
	// 2. Thêm state để theo dõi lỗi ảnh
	...
	return (
		<Link
			...
			{/* --- 3. SỬA PHẦN HIỂN THỊ ẢNH --- */}
			<div
				className="w-full relative bg-gray-200 overflow-hidden"
				style={{ paddingTop: "100%" }}>
				{/* A. Nếu có link ảnh VÀ chưa bị lỗi -> Render ảnh */}
				{displayImage && !imageError ? (
					<img
						...
						loading="lazy"
						// Quan trọng: Bắt lỗi ảnh ở đây
						onError={() => setImageError(true)}
					/>
				) : null}

				{/* B. Nếu KHÔNG có link HOẶC ảnh bị lỗi -> Render Icon */}
				{(!displayImage || imageError) && (
					<div className="absolute inset-0 h-full w-full flex items-center justify-center bg-gray-100 text-gray-400">
						<Shirt className="w-16 h-16 opacity-50" strokeWidth={1} />
					</div>
				)}

				{/* Badge độ giống (Giữ nguyên) */}
				{isSearchResult && (
