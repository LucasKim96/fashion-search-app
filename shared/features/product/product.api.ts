...
export const getPublicProducts = async (): Promise<ApiResponse<Product[]>> => {
    const res = await axiosInstance.get<ApiResponse<Product[]>>(PRODUCT_ENDPOINTS.PUBLIC_LIST);
    return res.data;
};

// Lấy danh sách sản phẩm public theo shop
export const getPublicProductsByShop = async (shopId: string): Promise<ApiResponse<Product[]>> => {
    const res = await axiosInstance.get<ApiResponse<Product[]>>(PRODUCT_ENDPOINTS.PUBLIC_LIST_BY_SHOP, {
        params: { shopId },
    });
    return res.data;
};
...
export const getProductDetail = async (productId: string): Promise<ApiResponse<Product>> => {
    const res = await axiosInstance.get<ApiResponse<Product>>(PRODUCT_ENDPOINTS.PUBLIC_DETAIL(productId));
    return res.data;
};

// Đếm số lượng sản phẩm public của shop
export const countProductsByShop = async (shopId: string): Promise<ApiResponse<number>> => {
    const res = await axiosInstance.get<ApiResponse<number>>(PRODUCT_ENDPOINTS.PUBLIC_COUNT_SHOP(shopId));
    return res.data;
};
...
export const getAllProductsAdmin = async (): Promise<ApiResponse<Product[]>> => {
    const res = await axiosInstance.get<ApiResponse<Product[]>>(PRODUCT_ENDPOINTS.ADMIN_LIST);
    return res.data;
};
...
export const getProductDetailAdmin = async (productId: string): Promise<ApiResponse<Product>> => {
    const res = await axiosInstance.get<ApiResponse<Product>>(PRODUCT_ENDPOINTS.ADMIN_DETAIL(productId));
    return res.data;
};

// Bật/tắt trạng thái sản phẩm (admin)
export const toggleProductActive = async (productId: string): Promise<ApiResponse<Product>> => {
    const res = await axiosInstance.patch<ApiResponse<Product>>(PRODUCT_ENDPOINTS.ADMIN_TOGGLE(productId));
    return res.data;
};

// Xóa sản phẩm (admin)
export const deleteProductAdmin = async (productId: string): Promise<ApiResponse<null>> => {
    const res = await axiosInstance.delete<ApiResponse<null>>(PRODUCT_ENDPOINTS.ADMIN_DELETE(productId));
    return res.data;
};
...
export const getShopProducts = async (): Promise<ApiResponse<Product[]>> => {
    const res = await axiosInstance.get<ApiResponse<Product[]>>(PRODUCT_ENDPOINTS.SHOP_LIST);
    return res.data;
};
...
export const createShopProduct = async (
    ...
    const res = await axiosInstance.post<ApiResponse<Product>>(PRODUCT_ENDPOINTS.SHOP_CREATE, formData, {
...
export const toggleShopProduct = async (productId: string): Promise<ApiResponse<Product>> => {
    const res = await axiosInstance.patch<ApiResponse<Product>>(PRODUCT_ENDPOINTS.SHOP_TOGGLE(productId));
    return res.data;
};

// Xóa sản phẩm (shop)
export const deleteShopProduct = async (productId: string): Promise<ApiResponse<null>> => {
    const res = await axiosInstance.delete<ApiResponse<null>>(PRODUCT_ENDPOINTS.SHOP_DELETE(productId));
    return res.data;
};
