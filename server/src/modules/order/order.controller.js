...
export const getShopOrderDetail = async (req, res, next) => {
	try {
		const { id } = req.params;
		const accountId = req.user?.id;
		...
		// Lấy shopId từ account đang đăng nhập
		const shopId = await getShopIdFromAccount(accountId);

		const result = await OrderService.getOrderDetailForShop(id, shopId);
...
export const markShipping = async (req, res, next) => {
	try {
		...
		return successResponse(res, result, "Đơn hàng đã chuyển sang 'shipping'");
	} catch (err) {
		next(err);
	}
};
...
export const markDelivered = async (req, res, next) => {
	try {
		...
		return successResponse(res, result, "Đã đánh dấu đơn hàng là 'delivered'");
	} catch (err) {
		next(err);
	}
};
...
export const adminCancelOrder = async (req, res, next) => {
	try {
		...
		validateObjectId(id, "ID đơn hàng");
		validateObjectId(adminId, "ID admin");
		const result = await OrderService.adminCancelOrder(id, adminId, reason);
