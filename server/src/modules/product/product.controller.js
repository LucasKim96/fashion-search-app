...
export const createProductWithVariants = async (req, res) => {
	...
	try {
		...
		const variantsWithImages = attachImagesByFileKey(
			req,
			"variantsPayload",
			tempFiles,
			{
	...
	try {
		...
		switch (mode) {
			case "keep":
				...
				break;
			case "add":
				newImages = await ProductService.handleAddModeImages(
					productId,
					keepImages,
					uploadedImages
				);
			...
			default:
				return res
					.status(400)
					.json({ success: false, message: "Mode không hợp lệ" });
		}

		// --- Gọi service cập nhật ---
		const result = await ProductService.updateProductImagesService(
			productId,
			newImages
		);
...
export const updateProductBasicInfo = async (req, res) => {
	try {
		const { productId } = req.params;
		const result = await ProductService.updateProductBasicInfoService(
			productId,
			req.body
		);
		return res.status(result.success ? 200 : 400).json(result);
...
export const toggleProductActiveAuto = async (req, res) => {
	try {
		const { productId } = req.params;
		const result = await ProductService.toggleProductActiveAutoService(
			productId
		);
		return res.status(result.success ? 200 : 400).json(result);
	} catch (error) {
...
export const deleteProductWithVariants = async (req, res) => {
	try {
		const { productId } = req.params;
		const result = await ProductService.deleteProductWithVariantsService(
			productId
		);
		return res.status(result.success ? 200 : 400).json(result);
	} catch (error) {
 ...
 */
export const handleCountProducts = async (
	req,
	res,
	{ isAdmin = false } = {}
) => {
...
export const searchProductsAdmin = (req, res) =>
	handleSearchProducts(req, res, true);
