...
export const generateVariantCombinations = async (req, res) => {
	try {
		...
		const result = await ProductVariantService.generateVariantCombinations(
			attributes
		);
...
export const generateNewVariantCombinations = async (req, res) => {
	try {
		...
		const result = await ProductVariantService.generateNewVariantCombinations(
			productId,
			attributes
		);
		res.status(result.success ? 200 : 400).json(result);
...
export const getProductAttributesWithValues = async (req, res) => {
	try {
		...
		const result = await ProductVariantService.getProductAttributesWithValues(
			productId,
			accountId
		);
		res.status(result.success ? 200 : 400).json(result);
...
export const createProductVariantsBulk = async (req, res) => {
	...
	try {
		...
		if (!Array.isArray(variantsPayload))
			return res
				.status(400)
				.json({ success: false, message: "variantsPayload phải là mảng" });
