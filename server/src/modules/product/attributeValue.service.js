...
export const createAttributeValues = async (
	...
	return withTransaction(async (session) => {
		...
		if (accountId) {
			...
			if (
				attribute.shopId &&
				attribute.shopId.toString() !== shopId.toString()
			) {
				throw new Error(
					"Bạn không có quyền thêm value cho thuộc tính của shop khác"
				);
			}
		...
		if (!Array.isArray(values) || values.length === 0)
			throw new Error("Danh sách values trống");
...
export const updateAttributeValue = async (
	...
	return withTransaction(async (session) => {
		try {
			...
			const attribute = await Attribute.findById(oldValue.attributeId).session(
				session
			);
			...
			if (accountId) {
				...
				if (
					!oldValue.shopId ||
					oldValue.shopId.toString() !== shop._id.toString()
				) {
...
export const toggleAttributeValueStatus = async (
	valueId,
	accountId = null,
	session
) => {
	try {
		...
		return {
			success: true,
			message: `Đã ${
				oldValue.isActive ? "kích hoạt" : "vô hiệu hóa"
			} giá trị thành công`,
			data: oldValue,
		};
	} catch (error) {
	...
	return withTransaction(async (session) => {
		try {
			...
			if (accountId) {
				...
				if (!attribute.isGlobal)
					throw new Error(
						"Admin chỉ được phép xóa giá trị thuộc tính toàn cục"
					);
			}
