 ...
 */
export const getShopAvailableAttributeWithValues = async (
	attributeId,
	accountId
) => {
	try {
...
export const deleteAttribute = async (id, accountId = null) => {
	return withTransaction(async (session) => {
		...
		try {
			...
			// === Xóa dữ liệu DB ===
			await AttributeValue.deleteMany({ attributeId: attribute._id }).session(
				session
			);
			...
			return {
				success: true,
				message: "Đã xóa thuộc tính và ảnh của các giá trị liên quan",
			};
		} catch (error) {
			// === Nếu lỗi → khôi phục ảnh từ backup ===
			backupPaths.forEach(({ backupPath, imagePath }) =>
				restoreFile(backupPath, imagePath)
			);
			throw error;
		}
...
export const toggleActiveAttribute = async (
	attributeId,
	accountId = null,
	session
) => {
	try {
		...
		if (accountId) {
			...
			if (attr.shopId?.toString() !== shop._id.toString())
				throw new Error(
					"Bạn không có quyền chỉnh sửa thuộc tính của shop khác!"
				);
		} else {
			// Admin
			if (!attr.isGlobal)
		...
		return {
			success: true,
			message: `Đã ${
				newStatus ? "kích hoạt" : "ẩn"
			} thuộc tính và toàn bộ giá trị liên quan`,
			data: attr,
		};
	} catch (error) {
