  ...
  try {
    ...
    if (action === "replace") {
      if (!roleIds.new || !Array.isArray(roleIds.new) || roleIds.new.length === 0) {
        throw new Error("Danh sách role mới (roleIds.new) không hợp lệ hoặc trống!");
      }
      validRoles = await Role.find({ _id: { $in: roleIds.new } });
      if (validRoles.length !== roleIds.new.length) throw new Error("Một hoặc nhiều vai trò mới không hợp lệ!");
    } else {
      if (!Array.isArray(roleIds) || roleIds.length === 0) {
        throw new Error("Danh sách roleIds không hợp lệ hoặc trống!");
      }
      validRoles = await Role.find({ _id: { $in: roleIds } });
      if (validRoles.length !== roleIds.length) throw new Error("Một hoặc nhiều vai trò không hợp lệ!");
    }
    ...
    if (action === "add") {
      for (const role of validRoles) {
        ...
        if (restrictedRoles.includes(role.roleName)) {
          const hasOtherRestricted = currentRoleNames.some(rn => restrictedRoles.includes(rn));
      ...
      for (const role of validRoles) {
        ...
        if (currentRoleNames.includes("Khách hàng") && currentRoleNames.includes("Chủ shop")) {
          if (role.roleName === "Khách hàng") {
            throw new Error("Không thể xóa vai trò 'Khách hàng' — đây là vai trò mặc định!");
          }
        }

        currentRoleIds = currentRoleIds.filter(id => id !== roleIdStr);
        currentRoleNames = currentRoleNames.filter(name => name !== role.roleName);
        removedCount++;
      }
      ...
      if (roleIds.old && Array.isArray(roleIds.old)) {
        for (const oldId of roleIds.old) {
          ...
          currentRoleNames = currentRoleNames.filter(name => name !== oldRole.roleName);
        }
      }
      ...
      throw new Error("Hành động không hợp lệ! (chỉ hỗ trợ 'add', 'remove' hoặc 'replace')");
    }
